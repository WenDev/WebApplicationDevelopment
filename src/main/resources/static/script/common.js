...
$.ajaxSetup({
  complete: function (b, a) {
    ...
    if (b.status == 401) {alertify.alert('您没有相关的操作权限')}
  }
})
...
function getRecursiveArray (b, c, a) {
  $.each(c, function (d, e) {
    b.push({ value: e.encodedId, text: e.name })
    if (e.children.length > 0) {getRecursiveArray(b, e.children, a + 1)}
  })
}

function getRecursiveArrayName (b, a) {
  if (b === null) {return ''}
  return getSubRecursiveArrayName({ children: b }, a)
}

function getSubRecursiveArrayName (c, b) {
  if (c.encodedId == b) {return c.name}
  var a = ''
...
function getSuffix (a) {
  if (a == '' || isUndefined(a)) {return ''}
  var c = a.lastIndexOf('.')
  var b = ''
  if (c != -1) {b = a.substring(c + 1)}
  return $.trim(b)
}

function getFileIcon (a) {
  var b = getSuffix(a)
  if (b == '') {return 'icon-file'}
  var c = 'icon-file'
...
Number.prototype.formatTime = function () {
  ...
  var d = function (e) {return (e >> 0) < 10 ? '0' + e : e}
...
function getRandFileName (b) {
  ...
  var a = c.length
  var d = ''
  for (i = 0; i < b; i++) {d += c.charAt(Math.floor(Math.random() * a))}
  return d
}