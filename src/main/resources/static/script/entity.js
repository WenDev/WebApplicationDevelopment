var entity = {
  ...
  loadEntityStatistics: function (a) {
    $.ajax({
      ...
      dataType: 'json',
      timeout: 60000,
      error: function (d, b, c) {alertify.alert('统计信息加载失败，请稍后再试')},
      success: function (b) {
        if (b.code <= 0) {
  ...
  initToggleAdvSearchMode: function () {
    ...
    $('#show-basic-search-btn').click(function () {$(this).closest('.search-item-container').removeClass('adv')})
  },
  initCustomFields: function (a, c) {
    var b = this
    $('#entity-fields-container .dropdown-menu-checkbox-list').click(function (d) {d.stopPropagation()})
    ...
    $('#change-fields-btn').click(function (f) {
      var d = $('#entity-fields-container input:checked[name=field-checkbox]').map(function () {return $(this).val()}).get().join()
  ...
  updateCustomFields: function (a, b, c) {
    $.ajax({
      ...
      type: 'POST',
      data: { category: a, ids: b },
      dataType: 'json',
      timeout: 60000,
      error: function (f, d, e) {alertify.alert('更新一览页面中显示字段信息失败，请稍后再试')},
      success: function (d) {
        if (d.code <= 0) {
  ...
  updateCustomFieldSort: function (a, c, b) {
    $.ajax({
      ...
      type: 'POST',
      data: { category: a, sortField: c, sortOrder: b },
      dataType: 'json',
      timeout: 60000,
      error: function (f, d, e) {alertify.alert('一览页面中排序字段信息更新失败，请稍后再试')},
      success: function (d) {
        if (d.code <= 0) {
          alertify.alert(d.message)
          return
  ...
  getEntityIcon: function (a) {
    if (a == ENTITY_CUSTOMER) {return 'company'}
    if (a == ENTITY_CONTACT) {return 'contacts'}
    if (a == ENTITY_DEAL) {return 'deal'}
    if (a == ENTITY_CONTRACT) {return 'contract'}
    if (a == ENTITY_PAYMENT) {return 'payment'}
    if (a == ENTITY_INVOICE) {return 'invoice'}
    if (a == ENTITY_PRODUCT) {return 'product'}
    if (a == ENTITY_PRODUCT_CATEGORY) {return 'product'}
    if (a == ENTITY_TASK) {return 'task'}
    if (a == ENTITY_WORKSHEET) {return 'worksheet'}
    if (a == ENTITY_STATISTICS) {return 'statistics'}
    if (a == ENTITY_CALLCENTER) {return 'tel'}
    return 'note2'
  },
  getEntityTypeName: function (a) {
    if (a == ENTITY_CUSTOMER) {return '客户'}
    if (a == ENTITY_CONTACT) {return '联系人'}
    if (a == ENTITY_DEAL) {return '销售机会'}
    if (a == ENTITY_CONTRACT) {return '合同'}
    if (a == ENTITY_PAYMENT) {return '款项'}
    if (a == ENTITY_INVOICE) {return '票据'}
    if (a == ENTITY_PRODUCT) {return '产品'}
    if (a == ENTITY_PRODUCT_CATEGORY) {return '产品分类'}
    if (a == ENTITY_TASK) {return '任务提醒'}
    if (a == ENTITY_WORKSHEET) {return '工单'}
    if (a == ENTITY_STATISTICS) {return '统计分析'}
    if (a == ENTITY_CALLCENTER) {return '呼叫中心'}
    return ''
  },
  getEntityTypeKey: function (a) {
    if (a == ENTITY_CUSTOMER) {return 'customer'}
    if (a == ENTITY_CONTACT) {return 'contact'}
    if (a == ENTITY_DEAL) {return 'deal'}
    if (a == ENTITY_CONTRACT) {return 'contract'}
    if (a == ENTITY_PAYMENT) {return 'payment'}
    if (a == ENTITY_INVOICE) {return 'invoice'}
    if (a == ENTITY_PRODUCT) {return 'product'}
    if (a == ENTITY_PRODUCT_CATEGORY) {return 'productCategory'}
    if (a == ENTITY_TASK) {return 'task'}
    if (a == ENTITY_WORKSHEET) {return 'worksheet'}
    if (a == ENTITY_STATISTICS) {return 'statistics'}
    if (a == ENTITY_CALLCENTER) {return 'callCenter'}
    return ''
  },
  getEntityUrl: function (b, a) {
    if (a == ENTITY_CUSTOMER) {return '/customer/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_CONTACT) {return '/contact/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_DEAL) {return '/deal/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_CONTRACT) {return '/contract/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_PAYMENT) {return '/payment/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_INVOICE) {return '/invoice/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_PRODUCT) {return '/product/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_PRODUCT_CATEGORY) {return '/product'}
    if (a == ENTITY_TASK) {return '/task/detail/' + encodeURIComponent(b)}
    if (a == ENTITY_WORKSHEET) {return '/worksheet/detail/' + encodeURIComponent(b)}
    return '#'
  },
  ...
  changeDeal: function (a) {
    if (!checkInput('form-change-deal')) {return}
    var e = this
    ...
    $.ajax({
      url: '/' + a + '/update/deal',
      type: 'POST',
      data: { id: _targetId, customerId: b, dealId: d },
      dataType: 'json',
      timeout: 60000,
  ...
  changeContract: function (a) {
    if (!checkInput('form-change-contract')) {return}
    var e = this
    var c = $('#change-deal-btn').button('loading')
    var b = $('#entity-change-customerId').val()
    ...
    $.ajax({
      ...
      type: 'POST',
      data: { id: _targetId, customerId: b, contractId: d },
      dataType: 'json',
      timeout: 60000,
      error: function (h, f, g) {
        c.button('reset')
  ...
  changeContact: function () {
    ...
    $.ajax({
      url: '/' + a + '/update',
      type: 'POST',
      data: { id: _targetId, value: c, pk: 'contactId' },
      dataType: 'json',
      timeout: 60000,
  ...
  initTaskModal: function () {
    $('#addUpdateTaskModal').on('shown.bs.modal', function (a) {$('#task-remark').focus()}).on('show.bs.modal', function (b) {
      var a = $(b.relatedTarget)
    ...
    $('#addUpdate-task-btn').click(function () {task.addUpdateTask()})
  },
  initChangeCustomerModal: function () {
    ...
    $('#change-customer-btn').click(function () {a.changeCustomer()})
  },
  changeCustomer: function () {
    if (!checkInput('form-change-customer')) {return}
    var d = this
    ...
    $.ajax({
      url: '/' + a + '/update',
      type: 'POST',
      data: { id: _targetId, value: b, pk: 'customerId' },
      dataType: 'json',
      timeout: 60000,
      error: function (g, e, f) {
        c.button('reset')
  ...
  transferCustomerToCommon: function () {
    ...
    var b = { publicGroupId: a }
    if (d) {b.ids = entity.selectEntityIds} else {b.id = _targetId}
  ...
  initDeleteEntitiesButton: function (b) {
    var a = this
    $('#entity-list-delete-btn').click(function () {a.deleteEntities($(this).data('name'), $(this).data('confirm'), $(this).data('containerid'), b)})
  },
  deleteEntities: function (a, b, c, e) {
    var d = this
    d.selectEntityIds = $('#' + c + ' input:checked[name=item-checkbox]').map(function () {return $(this).data('id')}).get().join()
    ...
    alertify.confirm(b, function () {
      $.ajax({
        url: '/' + a + '/delete/batch',
        type: 'POST',
        data: { ids: d.selectEntityIds },
        dataType: 'json',
        timeout: 60000,
        error: function (h, f, g) {alertify.alert('删除失败，请稍后再试')},
        success: function (f) {
          if (f.code <= 0) {
            alertify.alert(f.message)
            return
  ...
  deleteEntity: function (a, b, c) {
    alertify.confirm(b, function () {
      $.ajax({
        ...
        dataType: 'json',
        timeout: 60000,
        error: function (f, d, e) {alertify.alert('删除失败，请稍后再试')},
        success: function (d) {
          if (d.code <= 0) {
            alertify.alert(d.message)
            return
  ...
  initDetailTabs: function () {
    $('#entity-detail-tabs a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
      ...
      if (!_isSinglePage) {$('.slidePanel-scrollable').asScrollable('update')}
    })
  },
  showEncodedUserName: function (a) {
    ...
    $('.encodedUserId').each(function () {
      ...
      if (c != '') {$(this).text(b.getUserName(b.getUser(a, c)))}
    })
  },
  showEncodedGroupName: function (a) {
    ...
    $('.encodedGroupId').each(function () {
      var c = $(this).data('value')
      if (c != '') {$(this).text(getRecursiveArrayName(a, c))}
    })
  },
  initShareModal: function () {
    ...
    $('#shareSingleModal').on('show.bs.modal', function (b) {
      ...
      var c = new Array()
      $('#shareUser-link span.shareUser').each(function (d) {c.push($(this).data('value'))})
  ...
  shareEntities: function () {
    var e = $('#entity-shareUserId option:selected').map(function () {return $(this).val()}).get().join()
    if (e == '') {
    ...
    $.ajax({
      url: '/' + a + '/share/batch',
      type: 'POST',
      data: { ids: d.selectEntityIds, shareUserIds: e, shareType: b },
      dataType: 'json',
      timeout: 60000,
      error: function (h, f, g) {
        c.button('reset')
  ...
  shareSingleEntity: function () {
    ...
    $('#entity-single-shareUserId option:selected').each(function () {
      ...
      if (d == '') {d = $(this).val()} else {d = d + ',' + $(this).val()}
    })
    ...
    $.ajax({
      url: '/' + a + '/share',
      type: 'POST',
      data: { id: _targetId, shareUserIds: d },
      dataType: 'json',
      timeout: 60000,
      error: function (g, e, f) {
        c.button('reset')
      ...
      success: function (e) {
        ...
        $('#shareUser-link').html('')
        $.each(b, function (f, g) {g.appendTo($('#shareUser-link'))})
      }
    })
  },
  initTransferModal: function (b) {
    ...
    $('#transferModal').on('show.bs.modal', function (d) {
      ...
      if (isUndefined(c.data('target'))) {$('#transfer-entity-btn').data('batch', true)} else {$('#transfer-entity-btn').data('batch', false)}
    })
    $('#transfer-entity-btn').click(function () {a.transferEntity(b)})
  },
  transferEntity: function (h) {
    ...
    var d = '/' + a
    if (a == 'customer' && _visible == VISIBLE_PUBLIC) {d = d + '/transferPrivate'} else {d = d + '/transfer'}
    d = d + (e ? '/batch' : '')
    var b = { ownerUserId: c }
    if (e) {b.ids = g.selectEntityIds} else {b.id = _targetId}
    if (a == 'customer') {
    ...
    $.ajax({
      ...
      success: function (i) {
        ...
        if (!_isSinglePage && !e) {$.slidePanel.hide()}
        h()
      }
    })
  },
  initChangeNextContactDateModal: function () {
    var a = this
    $('#changeNextContactDateModal').on('show.bs.modal', function (b) {$('#form-changeNextContactDate-entity .form-group').removeClass('has-error')})
    $('#entity-changeNextContactDate-btn').click(function () {a.changeNextContactDate()})
  },
  changeNextContactDate: function () {
    ...
    var a = ''
    if (_targetType == ENTITY_CUSTOMER) {a = '/customer'} else {a = '/deal'}
    a = a + '/update/batch/nextContactDate'
    $.ajax({
      url: a,
      type: 'POST',
      data: { ids: entity.selectEntityIds, nextContactDate: c },
      dataType: 'json',
      timeout: 60000,
      error: function (g, e, f) {
        b.button('reset')
      ...
      success: function (e) {
        ...
        if (_targetType == ENTITY_CUSTOMER) {customerList.loadCustomer(customerList.currentPage)} else {dealList.loadDealByList(dealList.currentPage)}
      }
    })
  },
  initCustomModalButtons: function () {
    ...
    $('[data-plugin=\'customModal\']').click(function (b) {
      a.selectEntityIds = $('#entity-item-list input:checked[name=item-checkbox]').map(function () {return $(this).data('id')}).get().join()
  ...
  initSearchItem: function (c, b) {
    ...
    $('.search-item-condition-datetimepicker-dropdown-menu').click(function (d) {d.stopPropagation()})
  ...
  initTableDragger: function (a) {
    ...
    if (b == 0) {return}
    tableDragger(document.getElementById(a), { dragHandler: '.handle' }).on('drop', function (h, g, d, f) {
      var e = new Array()
  ...
  loadEntityExtInfo: function (b, a, c) {
    $.ajax({
      ...
      dataType: 'json',
      timeout: 60000,
      error: function (f, d, e) {alertify.alert('加载详细信息失败，请稍后再试')},
      success: function (d) {
        if (d.code <= 0) {
          alertify.alert(d.message)
          return
        ...
        $.each(d.items.extInfos, function (e, g) {
          ...
          if (i.data('type') == 'select' || i.data('type') == 'checklist') {i.data('value', g.value)}
          if (i.data('field-website')) {
            var f = g.value
            if (f.indexOf('http') < 0) {f = 'http://' + f}
            i.next().attr('href', f).removeClass('hide')
          }
          if (i.data('field-qq')) {
            var h = g.value
            i.next().attr('href', 'https://wpa.qq.com/msgrd?v=3&uin=' + h + '&site=qq&menu=yes').removeClass('hide').html('<img border=\'0\' src=\'/images/qq_talk.gif\' alt=\'点击这里给我发消息\' title=\'点击这里给我发消息\'/>')
          }
        })
        c()
      }
  ...
  initEditablePlugin: function (a) {
    ...
    $('[data-plugin=\'editable\']').editable({
      validate: function (e) {
        var d = $.trim(e)
        if ($(this).data('field-required')) {if (d == '') {return $(this).data('title') + '必须输入'}}
        if ($(this).data('field-number')) {if (d != '' && !$.isNumeric(d)) {return $(this).data('title') + '必须为数字'}}
        if ($(this).data('field-tel')) {if (d != '' && !isMobile(d) && !isTel(d)) {return $(this).data('title') + '电话格式不正确'}}
        if ($(this).data('field-email')) {if (d != '' && !isEmail(d)) {return $(this).data('title') + '邮箱格式不正确'}}
        if ($(this).data('field-website')) {if (d != '' && !isURL(d)) {return $(this).data('title') + '网址格式不正确，正确格式: http://www.qikebao.com'}}
        if (typeof $(this).data('field-date') !== 'undefined') {
          if ($.trim(e) != '') {
            var c = $(this).data('field-date')
            var b = $('#' + c).text()
            if ($.trim(b) != '') {if (c.indexOf('start') > 0) {if (moment($.trim(e)).isBefore(b)) {return '结束日期必须大于开始日期'}} else {if (moment(b).isBefore($.trim(e))) {return '开始日期必须小于结束日期'}}}
          }
        }
      },
      datepicker: { language: 'zh-CN' },
      combodate: { firstItem: 'name', minYear: moment().year() - 20, maxYear: moment().year() + 20, minuteStep: 1 },
      ajaxOptions: { dataType: 'json' },
      success: function (b, c) {
        if (!b) {return '更新过程中出现异常，请稍后再试'}
        if (b.code == 0) {return b.message}
        var d = $(this).data('callback')
        if (typeof window[d] === 'function') {window[d](c)}
      },
      error: function (b) {}
    })
  },
  loadSlidePanel: function (a) {
    $.slidePanel.show({ url: a, settings: { method: 'GET' } }, {
      direction: 'right',
      closeSelector: '.slidePanel-close',
      mouseDrag: false,
      touchDrag: false,
      pointerDrag: false,
      template: function (b) {return '<div class="' + b.classes.base + ' ' + b.classes.base + '-' + b.direction + '"><div class="' + b.classes.base + '-scrollable"><div><div class="' + b.classes.content + '"></div></div></div><div class="' + b.classes.base + '-handler"></div></div>'},
      ...
      loading: { template: function (b) {return '<div class="slidePanel-loading height-350 vertical-align text-center"><div class="loader-default loader vertical-align-middle" data-type="default"></div></div>'} }
    })
  },
  loadExtInfo: function (b, e, c, a) {
    ...
    var d = new Array()
    $.each(b.split(','), function (f, g) {if (!isNaN(g)) {d.push(g)}})
    if (d.length == 0) {return}
    $.ajax({
      url: c,
      type: 'POST',
      data: { ids: e.join(), fieldIds: d.join() },
      dataType: 'json',
      timeout: 60000,
      error: function (h, f, g) {},
      success: function (f) {
        if (f.code <= 0) {return}
        $.each(f.items.extInfos, function (g, h) {$('tr[id=\'' + a + h.encodedTargetId + '\']').find('.item-' + h.fieldId).text(h.value)})
      }
    })
  },
  initIndentSelect: function (c, d, a, b) {
    ...
    var g = this
    d.html('')
    var f = { encodedId: b, name: '全部', level: -1, children: c }
    if (c !== null) {g.addIndentSelectOption(d, f)}
    var e = {
      dropdownAutoWidth: true, templateResult: function (k, h) {
        var j = $(k.element).data('level')
        if (isUndefined(j) || j == '0') {return k.text}
        var i = $('<span><span class="padding-left-' + j + '0">' + k.text + '</span>')
        return i
      }
    }
    if (a != '') {e.dropdownParent = $('#' + a)}
    d.select2(e)
  },
  addIndentSelectOption: function (b, a) {
    var c = this
    $('<option></option>').val(a.encodedId).text(a.name).data('level', a.level + 1).appendTo(b)
    $.each(a.children, function (d, e) {c.addIndentSelectOption(b, e)})
  },
  ...
  getSearchCustomerSelectOption: function () {
    return {
      ...
      ajax: {
        ...
        dataType: 'json',
        delay: 250,
        data: function (a) {return { name: a.term, page: a.page }},
        processResults: function (a, b) {
          b.page = b.page || 1
          return { results: a.items.names }
        },
        cache: true
      },
  ...
  getUserAvatar: function (a) {
    if (a == '') {return '/images/person-avatar.png'}
    return a.imgPath
  },
  initUpdateNoteModal: function () {
    ...
    $('#update-note-btn').click(function (a) {note.startUploadUpdateFiles()})
  },
  initFieldSearch: function () {
    ...
    $('#search-dropdown-list .dropdown-menu-item a').click(function (c) {
      ...
      if (b == '2' || b == '3') {
        ...
        $.each(a, function (d, e) {$('#search-field-select').append(new Option(e, e, true, true))})
        return
      }
      if (b == '4' || b == '5') {
